...
window.PanelPerfil = {
  ...
  async _initFirebase() {
    try {
      ...
      this._firebaseApp = appModule.getApps().find(a => a.name === "[DEFAULT]")
      ...
      this.TIENDA_REF = this._doc(
        this.db, "Tiendas", this.LOCALIDAD_TIENDA, this.LOCALIDAD_TIENDA, this.TIENDA_ID
      );
  ...
  _initRealtime() {
    ...
    this._onSnapshot(
      this.TIENDA_REF,
      snap => {
        if (!snap.exists()) { this.showToast("⚠️ Documento no encontrado"); return; }
        ...
        if (sk) { sk.classList.add("hidden"); setTimeout(() => sk.remove(), 450); }
...
_updateAPPState() {
  ...
  populateUI(data) {
    ...
    this.setField("fieldTelefono",
      mc.llamada?.numero || "");
    this.setField("fieldWhatsapp",
      mc.whatsapp?.numero || "");
    this.setField("fieldInstagram",
      mc.instagram?.nombre
        ? (mc.instagram.nombre.startsWith("@") ? mc.instagram.nombre : "@" + mc.instagram.nombre)
        : "");
    ...
    if (imgs?.promociones) this.populatePromocionesGrid("promocionesGrid", imgs.promociones, 3);

    if (data.aforo_max !== undefined) this.setField("fieldAforo", data.aforo_max);
  ...
  renderSubcategorias() {
    ...
    lista.forEach(item => {
      ...
      if (this.selectedSubcats.includes(item.toLowerCase())) div.classList.add("selected");
  ...
  toggleSubcat(sub, el) {
    ...
    if (index >= 0) { this.selectedSubcats.splice(index, 1); el.classList.remove("selected"); }
    else            { this.selectedSubcats.push(value);       el.classList.add("selected");    }
    this.updateCatDisplay();
  ...
  updateCatDisplay() {
    ...
    if (this.selectedSubcats.length) text += ` • ${this.selectedSubcats.length} subcategorías`;
  ...
  _insertMapSaveBtn() {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        await this._updateDoc(lugarRef, { "ubicacion.latitud": lat, "ubicacion.longitud": lng });

        this.currentData.ubicacion = { ...this.currentData.ubicacion, latitud: lat, longitud: lng };
  ...
  initMapbox() {
    ...
    this.map.on("click", e => this._onMapPointChanged(e.lngLat.lat, e.lngLat.lng));
  },
  ...
  loadAvatar(url) {
    ...
    img.src = url;
    img.onload  = () => { skeleton.style.display = "none"; placeholder.style.display = "none"; img.classList.add("loaded"); };
    img.onerror = () => { skeleton.style.display = "none"; placeholder.style.display = "flex"; };
  },
    ...
    input.onchange = e => {
      ...
      reader.onload = async ev => {
        ...
        try {
          ...
          if (oldKey) {
            try {
              await this._deleteObject(this._storageRef(this._storage, `tiendas/${this.TIENDA_ID}/imagenes/promociones/${oldKey}.webp`));
            } catch { /* no existía */ }
          }
          ...
          this.populatePromocionesGrid("promocionesGrid",
            snapPromo.data()?.img_tienda?.lista_img?.promociones || {}, 3);
  ...
  async deleteFotoPromocion(key) {
    ...
    try {
      ...
      this.populatePromocionesGrid("promocionesGrid",
        snapDel.data()?.img_tienda?.lista_img?.promociones || {}, 3);
  ...
  _getContactName(method) {
    return { llamada: "Teléfono", whatsapp: "WhatsApp", instagram: "Instagram",
             facebook: "Facebook", tiktok: "TikTok", sitio_web: "Sitio web" }[method] || method;
  },
  ...
  async loadPublicidad() {
    ...
    try {
      ...
      console.error(e);
      container.innerHTML = '<div style="padding:40px;text-align:center;color:white;font-size:15px;">❌ Error cargando publicaciones.html</div>';
    }
  },
  ...
  _updateNameSilent(val) {
    ...
    document.getElementById("heroName")?.textContent    !== undefined && (document.getElementById("heroName").textContent    = v);
    document.getElementById("sidebarName")?.textContent !== undefined && (document.getElementById("sidebarName").textContent = v);
  },
  ...
  _initDraggableBtn() {
    ...
    const onStart = (clientX, clientY) => {
      ...
      btn.style.left = startLeft + "px"; btn.style.right  = "auto";
      btn.style.top  = startTop  + "px"; btn.style.bottom = "auto";
...
function enviarDatosAFrames() {
  ...
  document.querySelectorAll("iframe").forEach(frame => {
    frame.contentWindow?.postMessage({ type: "DATOS_TIENDA", payload }, window.location.origin);
  });
}
