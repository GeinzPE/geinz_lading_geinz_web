...
import {
  doc, setDoc, getDoc, getDocs, updateDoc, serverTimestamp,
...
function parseOpcionesStr(str) {
  ...
  return str.toString().split(';').map(s => s.trim()).filter(Boolean).map(part => {
    const [nombre, costo, stock] = part.split(':').map(x => (x ?? '').trim());
    return {
      nombre: nombre || '',
      activo: true,
...
function mapRowToRegistro(rawRow) {
  const row = {};
  Object.entries(rawRow).forEach(([k, v]) => { row[normKey(k)] = v; });
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (n && n.toString().trim() && o) {
      condiciones.push({ nombre: n.toString().trim(), opciones: parseOpcionesStr(o) });
    }
  }
...
function parseTextoPorBloques(texto) {
  ...
  const registros = [];
  let actual = null;
  const campoRegex = /^(nombre|producto|precio|descripcion|descripción|stock|categoria|categoría)\s*[:\-]\s*(.+)$/i;
  ...
  lineas.forEach(linea => {
    ...
    if (campo === 'nombre' || campo === 'producto') {
      ...
      actual = { _selected: true, categoria: '', nombre: valor, descripcion: '', precio: '', stock: '', disponible: true, imagenes: [], condiciones: [] };
    } else if (actual) {
      if (campo === 'precio') actual.precio = valor.replace(/[^0-9.,]/g, '').replace(',', '.');
  ...
  if (tabla) {
    ...
    if (filas.length > 1) {
      ...
      return filas.slice(1).map(celdas => {
        const raw = {};
        headers.forEach((h, i) => { raw[h] = celdas[i] || ''; });
...
async function scrapearURL(url, usarProxy) {
  const target = usarProxy ? `https://corsproxy.io/?${encodeURIComponent(url)}` : url;
  const resp = await fetch(target);
  if (!resp.ok) throw new Error('No se pudo acceder a la URL (código ' + resp.status + ').');
  const html = await resp.text();
  const parsedDoc = new DOMParser().parseFromString(html, 'text/html');
  const registros = [];

  // 1) JSON-LD (schema.org Product) — la fuente más confiable si existe
  parsedDoc.querySelectorAll('script[type="application/ld+json"]').forEach(tag => {
    try {
      ...
      items.forEach(item => {
        if (item && /product/i.test(item['@type'] || '')) {
          ...
          const imagenRaw = Array.isArray(item.image) ? item.image[0] : item.image;
          registros.push({
            _selected: true,
            categoria: '',
            nombre: item.name || '',
    ...
    } catch (e) { /* JSON-LD inválido, se ignora ese bloque */ }
  });

  // 2) Fallback heurístico: tarjetas con clases típicas de tienda online
  if (!registros.length) {
    const candidatos = parsedDoc.querySelectorAll('[class*="product" i], [class*="item" i]');
    candidatos.forEach(el => {
      if (registros.length >= 100) return; // límite de seguridad
      const nombreEl = el.querySelector('h1,h2,h3,[class*="title" i],[class*="name" i]');
      ...
      if (nombreEl && precioEl && nombreEl.textContent.trim()) {
        ...
        if (imgEl) {
          ...
          try { imagenUrl = new URL(src, url).href; } catch (e) { imagenUrl = ''; }
        }
        registros.push({
          _selected: true,
          categoria: '',
          ...
          descripcion: '',
          precio: precioEl.textContent.replace(/[^\d.,]/g, '').replace(',', '.'),
          stock: '',
          disponible: true,
...
async function subirImagenDesdeURL(url, productoId, storage, storagePathProducto) {
  try {
    const resp = await fetch(url, { mode: 'cors' });
    if (!resp.ok) throw new Error('status ' + resp.status);
    const blob = await resp.blob();
    const filename = (url.split('/').pop() || 'img').split('?')[0] || 'imagen.jpg';
...
async function ejecutarImportacion(deps, registros, categoriaDefault, onProgress) {
  const { categoriasRef, productosRef, storage, storagePathProducto, toast } = deps;
  const categoriasCreadas = new Set();
  const seleccionados = registros.filter(r => r._selected && r.nombre && r.nombre.trim());
  let ok = 0, fail = 0, i = 0;
  ...
  for (const reg of seleccionados) {
    ...
    try {
      const nombreCategoria = (reg.categoria && reg.categoria.trim()) || categoriaDefault;
      ...
      if (!categoriasCreadas.has(nombreCategoria)) {
        ...
        if (!snap.exists()) {
          await setDoc(catRef, { nombre: nombreCategoria, createdAt: serverTimestamp() });
        }
        categoriasCreadas.add(nombreCategoria);
      }

      const nuevoRef = doc(productosRef(nombreCategoria));
      const precio = parseFloat((reg.precio || '0').toString().replace(',', '.')) || 0;
      const stock = (reg.stock === '' || reg.stock === undefined || reg.stock === null)
        ? null : Math.max(0, parseInt(reg.stock, 10) || 0);
      ...
      if (imgs.length) {
        ...
        for (const url of imgs) {
          subidas.push(await subirImagenDesdeURL(url, nuevoRef.id, storage, storagePathProducto));
        }
  ...
  toast(`Importación terminada: ${ok} producto(s) creado(s)${fail ? `, ${fail} con errores` : ''}.`);
}
...
export function initImportador(deps) {
  ...
  btnImportar.className = 'inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 text-purple-100 font-medium text-sm px-4 py-2.5 border border-purple-900/30 transition-all active:scale-95 shrink-0';
  btnImportar.innerHTML = `
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M7 10l5 5 5-5"/><path d="M12 15V3"/>
    </svg>
    <span>Importar productos</span>`;
  if (btnNuevaCat && btnNuevaCat.parentNode) {
  ...
  overlay.className = 'fixed inset-0 z-[150] items-center justify-center p-4 bg-black/80 backdrop-blur-sm';
  overlay.innerHTML = `
    <div class="w-full max-w-3xl rounded-2xl bg-[#0d0a17] border border-purple-900/30 shadow-2xl p-6 max-h-[90vh] overflow-y-auto thin-scroll">
      ...
      <div class="flex flex-wrap gap-2 mb-5" id="importar-tabs">
        <button type="button" data-tab="excel" class="import-tab-btn active-import-tab rounded-xl px-3 py-1.5 text-xs font-medium bg-violet-950/50 border border-violet-500/40 text-violet-200">Excel / CSV</button>
        <button type="button" data-tab="word" class="import-tab-btn rounded-xl px-3 py-1.5 text-xs font-medium bg-white/5 border border-purple-900/30 text-purple-300">Word (.docx)</button>
        <button type="button" data-tab="pdf" class="import-tab-btn rounded-xl px-3 py-1.5 text-xs font-medium bg-white/5 border border-purple-900/30 text-purple-300">PDF</button>
        <button type="button" data-tab="url" class="import-tab-btn rounded-xl px-3 py-1.5 text-xs font-medium bg-white/5 border border-purple-900/30 text-purple-300">Desde una URL</button>
      </div>

      <!-- Panel Excel -->
      <div class="import-panel" data-panel="excel">
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center mb-2">
          <input type="file" id="input-import-excel" accept=".xlsx,.xls,.csv"
            class="text-xs text-purple-200 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-800 file:text-white file:px-3 file:py-2 file:text-xs file:font-medium">
          <button type="button" id="btn-analizar-excel" class="rounded-xl px-3 py-2 text-xs font-medium text-white bg-violet-800 hover:bg-violet-700">Analizar archivo</button>
          <button type="button" id="btn-descargar-plantilla" class="rounded-xl px-3 py-2 text-xs font-medium text-violet-300 hover:text-violet-200 bg-white/5 border border-purple-900/30">⬇ Descargar plantilla</button>
        </div>
      ...
      <div class="import-panel hidden" data-panel="word">
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center mb-2">
          <input type="file" id="input-import-word" accept=".docx"
            class="text-xs text-purple-200 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-800 file:text-white file:px-3 file:py-2 file:text-xs file:font-medium">
          <button type="button" id="btn-analizar-word" class="rounded-xl px-3 py-2 text-xs font-medium text-white bg-violet-800 hover:bg-violet-700">Analizar archivo</button>
        </div>
      ...
      <div class="import-panel hidden" data-panel="pdf">
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center mb-2">
          <input type="file" id="input-import-pdf" accept=".pdf"
            class="text-xs text-purple-200 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-800 file:text-white file:px-3 file:py-2 file:text-xs file:font-medium">
          <button type="button" id="btn-analizar-pdf" class="rounded-xl px-3 py-2 text-xs font-medium text-white bg-violet-800 hover:bg-violet-700">Analizar archivo</button>
        </div>
      ...
      <!-- Panel URL -->
      <div class="import-panel hidden" data-panel="url">
        <div class="flex flex-col sm:flex-row gap-3 items-center mb-2">
          <input type="text" id="input-import-url" placeholder="https://tuweb.com/catalogo"
            class="flex-1 rounded-xl bg-[#05040a] border border-purple-900/30 px-3.5 py-2.5 text-sm text-purple-100 placeholder-purple-400/30 outline-none focus:border-violet-600">
          <button type="button" id="btn-analizar-url" class="rounded-xl px-3 py-2 text-xs font-medium text-white bg-violet-800 hover:bg-violet-700 shrink-0">Extraer productos</button>
        </div>
        <label class="flex items-center gap-2 text-[11px] text-purple-300/60 mb-2">
          <input type="checkbox" id="input-usar-proxy" class="accent-violet-600">
          Usar proxy CORS si la carga directa falla (el contenido pasa por un tercero: corsproxy.io)
        </label>
        <p class="text-[11px] text-purple-300/50 leading-relaxed">El resultado depende de cómo esté armada la página: funciona mejor en tiendas con datos estructurados (schema.org). No hay garantía de que funcione en cualquier sitio.</p>
      </div>
  ...
  btnImportar.addEventListener('click', async () => {
    ...
    } catch (e) { categoriasExistentes = []; }
  });
  overlay.querySelector('#btn-cerrar-importar').addEventListener('click', cerrar);
  overlay.querySelector('#btn-cancelar-importar').addEventListener('click', cerrar);
  ...
  overlay.querySelectorAll('.import-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      overlay.querySelectorAll('.import-tab-btn').forEach(b => {
        b.classList.remove('active-import-tab', 'bg-violet-950/50', 'border-violet-500/40', 'text-violet-200');
        b.classList.add('bg-white/5', 'border-purple-900/30', 'text-purple-300');
      });
      btn.classList.add('active-import-tab', 'bg-violet-950/50', 'border-violet-500/40', 'text-violet-200');
      btn.classList.remove('bg-white/5', 'border-purple-900/30', 'text-purple-300');
      overlay.querySelectorAll('.import-panel').forEach(p => p.classList.add('hidden'));
      overlay.querySelector(`.import-panel[data-panel="${btn.dataset.tab}"]`).classList.remove('hidden');
    });
  });
  ...
  function mostrarPreview(registros) {
    ...
    if (!registros.length) {
      setEstado('No se detectó ningún producto. Revisa el archivo/URL o prueba con Excel.', true);
      config.classList.add('hidden');
      btnConfirmar.disabled = true;
      return;
    }

    setEstado(`Se detectaron ${registros.length} producto(s). Revisa y ajusta antes de importar.`);

    // Categorías: existentes + las nuevas que trae el propio archivo
    const nuevasDelArchivo = [...new Set(registros.map(r => r.categoria).filter(Boolean))];
    const todasCategorias = [...new Set([...categoriasExistentes, ...nuevasDelArchivo])];
    selectDefault.innerHTML = todasCategorias.map(c => `<option value="${c}">${c}</option>`).join('')
  ...
  overlay.querySelector('#btn-descargar-plantilla').addEventListener('click', (e) => {
    conBotonCargando(e.currentTarget, descargarPlantillaExcel);
  });

  overlay.querySelector('#btn-analizar-excel').addEventListener('click', (e) => {
    const file = overlay.querySelector('#input-import-excel').files[0];
    if (!file) { setEstado('Selecciona un archivo primero.', true); return; }
    conBotonCargando(e.currentTarget, async () => {
      ...
      } catch (err) { console.error(err); setEstado('No se pudo leer el archivo: ' + err.message, true); }
    });
  });
  ...
  overlay.querySelector('#btn-analizar-word').addEventListener('click', (e) => {
    ...
    if (!file) { setEstado('Selecciona un archivo .docx primero.', true); return; }
    conBotonCargando(e.currentTarget, async () => {
      try {
        setEstado('Leyendo documento...');
        const registros = await parseWord(file);
        mostrarPreview(registros);
      } catch (err) { console.error(err); setEstado('No se pudo leer el documento: ' + err.message, true); }
    });
  });

  overlay.querySelector('#btn-analizar-pdf').addEventListener('click', (e) => {
    const file = overlay.querySelector('#input-import-pdf').files[0];
    if (!file) { setEstado('Selecciona un archivo PDF primero.', true); return; }
    conBotonCargando(e.currentTarget, async () => {
      try {
        setEstado('Leyendo PDF...');
        const registros = await parsePDF(file);
        mostrarPreview(registros);
      } catch (err) { console.error(err); setEstado('No se pudo leer el PDF: ' + err.message, true); }
    });
  });

  overlay.querySelector('#btn-analizar-url').addEventListener('click', (e) => {
    const url = overlay.querySelector('#input-import-url').value.trim();
    const usarProxy = overlay.querySelector('#input-usar-proxy').checked;
    if (!url) { setEstado('Escribe una URL primero.', true); return; }
    conBotonCargando(e.currentTarget, async () => {
      try {
        setEstado('Extrayendo productos de la página...');
        const registros = await scrapearURL(url, usarProxy);
        mostrarPreview(registros);
      } catch (err) {
        console.error(err);
        setEstado('No se pudo leer la URL directamente (posible bloqueo CORS). Prueba activando "Usar proxy CORS" o usa Excel.', true);
      }
    });
  });

  /* ---- Confirmar importación ---- */
  overlay.querySelector('#btn-confirmar-importar').addEventListener('click', async () => {
    const btn = overlay.querySelector('#btn-confirmar-importar');
    const label = overlay.querySelector('#btn-confirmar-importar-label');
    const categoriaDefault = overlay.querySelector('#select-categoria-default').value;
    const original = label.textContent;
    btn.disabled = true;
    try {
      await ejecutarImportacion(deps, registrosActuales, categoriaDefault, (i, total) => {
