...
const categoriaParam = (params.get('categoria') || '').replace(/\+/g, ' ').toLowerCase().trim();
...
function getEstado(horario) {
    ...
    if (bloques.length > 0) {
        ...
        if (bloque.cerrado === true) return { estado: 'cerrado', texto: 'Cerrado', clase: 'closed' };

        const now = new Date();
        const [hA, mA] = (bloque.h_apertura || '00:00').split(':').map(Number);
        const [hC, mC] = (bloque.h_cierre || '23:59').split(':').map(Number);
        const apert = new Date(now); apert.setHours(hA, mA, 0);
        const cierr = new Date(now); cierr.setHours(hC, mC, 0);

        if (now >= apert && now < cierr) {
            const diffMs = cierr - now;
            const diffH = Math.floor(diffMs / 3600000);
            const diffM = Math.floor((diffMs % 3600000) / 60000);
            return { estado: 'abierto', texto: diffH > 0 ? `Cierra en ${diffH}h ${diffM}m` : `Cierra en ${diffM}m`, clase: diffMs < 3600000 ? 'warn' : 'open' };
        } else if (now < apert) {
            return { estado: 'pronto', texto: `Abre hoy a las ${bloque.h_apertura}`, clase: 'warn' };
        } else {
...
function renderChips() {
    ...
    all.onclick = () => { changeCategory('todos'); };
...
function renderList() {
    ...
    if (activeSubcat !== 'todos') {
        lista = lista.filter(ti => (ti.subcategoria || []).some(s => norm(s) === norm(activeSubcat)));
    }
    ...
    metaCountEl.textContent = total > 6 ? `Mostrando 6 de ${total}` : `${total} resultado${total !== 1 ? 's' : ''}`;
    ...
    listEl.innerHTML = '';
    lista.forEach((ti) => {
        const estado = ti._estado || { estado: 'cerrado', texto: 'Cerrado', clase: 'closed' };
        ...
        const logoEl = ti.logo
            ...
            : '';
        const iniciales = (ti.nombre_tienda || '?').split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase();
        const placeholderDisplay = ti.logo ? 'none' : 'flex';
        const subcatsHtml = (ti.subcategoria || []).slice(0, 3).map(s => `<span class="subcat-tag">${s.charAt(0).toUpperCase() + s.slice(1)}</span>`).join('');
        ...
        card.addEventListener('click', () => {
            const cat = encodeURIComponent((ti.categoria_tienda || categoriaParam).replace(/\s+/g, '+'));
            window.location.href = `/redirect/perfil_negocio.html?id=${ti.id}&l=${localidad}&c=${cat}`;
        });
...
async function cargar() {
    try {
        ...
        snap.forEach(docSnap => {
            ...
            if (categoriaParam && norm(d.categoria_tienda) !== norm(categoriaParam)) return;
        ...
        allTiendas.forEach(ti => (ti.subcategoria || []).forEach(s => subcatSet.add(s.toLowerCase())));
