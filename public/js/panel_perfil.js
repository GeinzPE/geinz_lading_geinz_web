...
function initRealtime() {
    ...
    onSnapshot(TIENDA_REF, (snap) => {
        if (!snap.exists()) { showToast("⚠️ Documento no encontrado"); return; }
...
function setSwitchAuto(switchId, isEnabled) {
    const switchElement = document.querySelector(`input[data-method="${switchId}"]`);
    if (switchElement) {
...
async function togglePayMethod(method, enabled) {
    try {
        ...
        showToast(`${method.toUpperCase()} ${enabled ? "activado" : "desactivado"}`);
        showSaveFab();
    } catch (e) {
...
function populateUI(data) {
    ...
    loadAvatar(data.logo_tienda || "");

    // Categoría
    if (data.categoria_tienda) {
        selectedCat = data.categoria_tienda;
        document.querySelectorAll("#catMain .cat-chip").forEach(c =>
            c.classList.toggle("selected", c.textContent.toLowerCase().includes(selectedCat.toLowerCase()))
        );
    }
    if (Array.isArray(data.subcategoria) && data.subcategoria.length) {
        selectedSubcat = data.subcategoria[0];
        document.querySelectorAll("#catSub .cat-chip").forEach(c =>
            c.classList.toggle("selected", c.textContent.toLowerCase().includes(selectedSubcat.toLowerCase()))
        );
    }
    updateCatDisplay();
    ...
    if (data.metodo_contacto) {
        ...
        setField("fieldInstagram", mc.instagram?.nombre ? (mc.instagram.nombre.startsWith('@') ? mc.instagram.nombre : '@' + mc.instagram.nombre) : "");
    ...
    if (imgs?.servicios_productos) populatePhotoGrid("productosGrid", imgs.servicios_productos, 6);
    if (imgs?.promociones) populatePhotoGrid("promocionesGrid", Object.values(imgs.promociones), 3);
...
async function collectAndSave() {
    ...
    updates["metodo_contacto.instagram.nombre"] = (g("fieldInstagram") || "").replace("@", "");
...
function selectCat(el, val) {
    document.querySelectorAll("#catMain .cat-chip").forEach(c => c.classList.remove("selected"));
...
function selectSubcat(el, val) {
    document.querySelectorAll("#catSub .cat-chip").forEach(c => c.classList.remove("selected"));
    el.classList.add("selected");
    selectedSubcat = val;
    updateCatDisplay();
    showSaveFab();
...
function updateCatDisplay() {
    const text = [selectedCat, selectedSubcat].filter(Boolean).join(" › ") || "Sin seleccionar";
    const d = document.getElementById("catDisplay");
    if (d) d.textContent = text;
    const h = document.getElementById("heroCat");
    if (h) h.textContent = selectedCat ? "📂 " + text + " ›" : "📂 Sin categoría ›";
}
...
function addPromo() {
    ...
    ["promoTitle", "promoDesc", "promoDiscount"].forEach(id => document.getElementById(id).value = "");
...
function setContactSwitch(contactId, isEnabled) {
    const switchElement = document.querySelector(`input[data-contact="${contactId}"]`);
    if (switchElement) {
        switchElement.checked = isEnabled === true;
    }
}
...
async function toggleContactMethod(method, enabled) {
    try {
        ...
        showToast(`${getContactName(method)} ${enabled ? "activado" : "desactivado"}`);
        showSaveFab();
    } catch (e) {
...
Object.assign(window, {
    showSection, toggleExpand, openExpandable, toggleDay,
    updateName, updateDesc, focusField,
    selectCat, selectSubcat,
    openModal, closeModal,
    applyProfileImg, applyAmbienteImg,
    addProduct, addPromo,
    saveChanges, showToast, toggleSidebar, onScheduleChange,
    togglePayMethod, toggleContactMethod, toggleLlamadaMethod  // ← añade toggleLlamadaMethod
});